...
7. Create empty `reports/daily/` and `docs/` directories, preserving them with `.gitkeep` if needed.
...
Purpose: render Markdown files to GitHub Pages HTML under `docs/`.
...
Use `docs/` as the static site output directory.
...
- GitHub Pages can publish from `docs/`.
...
```bash
git add index.md data entities reports docs
